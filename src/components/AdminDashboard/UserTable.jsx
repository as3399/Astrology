...
import "./Imports";
import dp from "../images/user/11.png";
...
export default function UserTable() {
  ...
  useEffect(() => {
    ...
    if (search.text == "" && Sorted == false) {
      getUsers(Page);
    }
    if (search.text == "" && Sorted == true) {
      SortUsers();
    }
  }, [search]);
  ...
  const ActivateStatus = async (i) => {
    await axios
      ...
      .then((resp) => {
        ...
        if (Sorted == true && search.text == "") {
          SortUsers();
        }
        if (search.text !== "") {
          SearchAPI();
        } 
        if (Sorted == false && search.text == ""){
          getUsers(Page);
        }
      });
  };
